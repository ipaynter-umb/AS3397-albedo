...
# VNP file name
vnp_file_name = 'VNP43MA3.A2021201.h17v01.002.2022121145105.h5'
# VJ1 file name
vj1_file_name = 'VJ143MA3.A2021201.h17v01.002.2022153174223.h5'
...
# Split out the tile name and the date
...
# Path for results
results_path = Path(environ['output_files_path'] + f'{year}_{doy}_{tile}/')
...
for key in vnp_file['HDFEOS']['GRIDS']['VIIRS_Grid_BRDF']['Data Fields'].keys():
    # If the key begins with Albedo
    if key.split('_')[0] == "Albedo":
        ...
        diff_array = np.multiply(np.subtract(vnp_filter_array, vj1_filter_array), 0.001)
        ...
        vnp_filter_array = np.multiply(vnp_filter_array, 0.001)
        vj1_filter_array = np.multiply(vj1_filter_array, 0.001)
        ...
        ax.set_title(f"Difference (VNP43A3 - VJ143A3)")
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
        # ax.axis('off')
        ...
        plt.legend(labels=["VNP43MA3", "VJ143MA3"])
        ...
        ax_hexbin.set_xlabel('VNP43A3 Albedo')
        ax_hexbin.set_ylabel('VJ143A3 Albedo')
        ...
        ax.set_xlabel(f"Difference (VNP43A3 - VJ143A3)")
